...
void InitScore(void)
{
	...
	for (nCntScore = 0; nCntScore < MAX_SCORE_POLYGON; nCntScore++)
	{
		...
		{
			g_aScorePolygon[nCntScore].pos = D3DXVECTOR3(SCORE_RESULT_POS_WIDTH - (SCORE_POLYGON_WIDTH*(MAX_SCORE_POLYGON - nCntScore) + (SCORE_POLYGON_WIDTH / 2)), SCORE_RESULT_POS_HEIGHT, 0.0f);
...
void UpdateScore(void)
{
	...
	for (nCntScore = 0; nCntScore < MAX_SCORE_POLYGON; nCntScore++)
	{
		...
		if (nCntScore == 0)
		{
			pVtx[0].tex = D3DXVECTOR2(g_aScore[0] / (int)powf(10, 7)*0.1f, 0.0f);
			pVtx[1].tex = D3DXVECTOR2(g_aScore[0] / (int)powf(10, 7)*0.1f + 0.1f, 0.0f);
			pVtx[2].tex = D3DXVECTOR2(g_aScore[0] / (int)powf(10, 7)*0.1f, 1.0f);
			pVtx[3].tex = D3DXVECTOR2(g_aScore[0] / (int)powf(10, 7)*0.1f + 0.1f, 1.0f);
		}
		else if (nCntScore == 7)
		{
			pVtx[0].tex = D3DXVECTOR2(g_aScore[0] % 10 * 0.1f, 0.0f);
			pVtx[1].tex = D3DXVECTOR2(g_aScore[0] % 10 * 0.1f + 0.1f, 0.0f);
			pVtx[2].tex = D3DXVECTOR2(g_aScore[0] % 10 * 0.1f, 1.0f);
			pVtx[3].tex = D3DXVECTOR2(g_aScore[0] % 10 * 0.1f + 0.1f, 1.0f);
		}
		else
		{
			pVtx[0].tex = D3DXVECTOR2(g_aScore[0] % (int)powf(10, MAX_SCORE_POLYGON - nCntScore) / (int)powf(10, 7 - nCntScore) *0.1f, 0.0f);
			pVtx[1].tex = D3DXVECTOR2(g_aScore[0] % (int)powf(10, MAX_SCORE_POLYGON - nCntScore) / (int)powf(10, 7 - nCntScore)*0.1f + 0.1f, 0.0f);
			pVtx[2].tex = D3DXVECTOR2(g_aScore[0] % (int)powf(10, MAX_SCORE_POLYGON - nCntScore) / (int)powf(10, 7 - nCntScore)*0.1f, 1.0f);
			pVtx[3].tex = D3DXVECTOR2(g_aScore[0] % (int)powf(10, MAX_SCORE_POLYGON - nCntScore) / (int)powf(10, 7 - nCntScore)*0.1f + 0.1f, 1.0f);
		}
	...
	if (GetMode() == MODE_RESULT)
	{
		if (g_aScore[0] >= g_aScore[1])
		{
			SetHiScore(g_aScore[0]);
		}
	}
}
