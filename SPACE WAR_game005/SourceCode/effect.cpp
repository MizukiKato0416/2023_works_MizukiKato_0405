...
void UpdateEffect(void)
{
	for (int nCntEffect = 0; nCntEffect < MAX_EFFECT; nCntEffect++)
	{
		if (g_aEffect[nCntEffect].bUse == true)
		{
			...
			else if (g_aEffect[nCntEffect].nType == EFFEFCT_MOVETYPE_PLAYER_HOVER)
			{									 
				g_aEffect[nCntEffect].scale.x -= 2.0f;
				g_aEffect[nCntEffect].scale.y -= 2.0f;

				if (g_aEffect[nCntEffect].scale.x <= 0.0f || g_aEffect[nCntEffect].scale.y <= 0.0f)
				{
					g_aEffect[nCntEffect].bUse = false;
				}
			}
			else if (g_aEffect[nCntEffect].nType == EFFEFCT_MOVETYPE_PLAYER_LIFE_LOW)
			{
				g_aEffect[nCntEffect].move.y += 0.2f;
				g_aEffect[nCntEffect].move.x += (float(rand() % 12 + -6) / 10);
				g_aEffect[nCntEffect].move.z += (float(rand() % 12 + -6) / 10);
			}
		}
	}
