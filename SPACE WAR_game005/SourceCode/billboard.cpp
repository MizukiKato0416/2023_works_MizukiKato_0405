...
void SetPositionBillboard(D3DXVECTOR3 pos, bool bUse)
{
	Camera *pCamera = GetCamera();
	if (g_aBillboard[0].bUse == true)
	{
		g_aBillboard[0].pos.x = pos.x;
		g_aBillboard[0].pos.y = pos.y;
		g_aBillboard[0].pos.z = pos.z;

		if (bUse == false)
		{
			g_aBillboard[0].bUse = false;
