...
//マクロ定義
//================================================
#define GAME03_RECOVERYCOUNT	(20)		//回復する頻度
#define GAME03_NEXT_SCENE_COUNT	(180)		//次のシーンまでのカウント
#define GAME03_GAME_OVER_SIZE_X	(914.0f)	//ゲームオーバーのサイズX
#define GAME03_GAME_OVER_SIZE_Y	(128.0f)	//ゲームオーバーのサイズY
...
//デフォルトコンストラクタ
//================================================
CGame03::CGame03(int nPriority):CObject(nPriority)
{
	m_pHpGauge = nullptr;
	m_pMpGauge = nullptr;
	m_pSpGauge = nullptr;
...
//初期化処理
//================================================
HRESULT CGame03::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size)
{
	//サウンド取得処理
	CSound *pSound;
	...
	pDungeonBossRoadBuilder = CDungeonBossRoadBuilder::Create();
	//敵がいる部屋
	...
	//ロード処理
	FILE *pFile;
	pFile = fopen("data/STAGE/stage03.csv", "r");
	int nCntX = 0, nCntZ = 0;
	char aLine[64];
	int nDepth = 5;		//縦の数
	int nWidth = 5;		//横の数
	...
	for (nCntZ = 0; nCntZ < nDepth ; nCntZ++)
	{
		for (nCntX = 0; nCntX < nWidth ; nCntX++)
		{
			...
			else if (aMapData[nCntZ][nCntX] == 6)
			{
				//道だけ
			...
			else if (aMapData[nCntZ][nCntX] == 7)
			{
				//ボスがいる道
				CDungeonRoomDirector::Create(pDungeonBossRoadBuilder, D3DXVECTOR3(DUNGEON_ROOM_BUILDER_SIZE * (nCntX - (nWidth - 1) / 2.0f),
							                                                       0.0f, DUNGEON_ROOM_BUILDER_SIZE * (nCntZ - (nDepth - 1) / 2.0f)));
				//道のマップを作る
				CMap::Create(D3DXVECTOR3(MAP_ROOM_SIZE * (nCntX - (nWidth - 1) / 2.0f),
					                     -MAP_ROOM_SIZE * (nCntZ - (nDepth - 1) / 2.0f),
	...
	while (pObject != nullptr)
	{
		...
		//オブジェクトの種類が敵だったら
		if (pObject->GetObjType() == CObject::OBJTYPE::ENEMY_BEE ||
...
//更新処理
//================================================
void CGame03::Update(void)
{
	//カウンターを加算
	m_nRecoveryCounter++;
	//既定の値以上になったら
	if (m_nRecoveryCounter >= GAME03_RECOVERYCOUNT)
	{
		//0にする
		m_nRecoveryCounter = 0;
		//HPとMPを回復させる
...
void CGame03::GameOver(void)
{
	...
	if (m_pObjet2DGame == nullptr)
	{
		m_pObjet2DGame = CObject2D::Create(D3DXVECTOR3(SCREEN_WIDTH / 2.0f, SCREEN_HEIGHT / 2.0f, 0.0f),
			                               D3DXVECTOR3(GAME03_GAME_OVER_SIZE_X, GAME03_GAME_OVER_SIZE_Y, 0.0f), CObject::PRIORITY_GAME_OVER);
		//ゲームオーバーの時
		if (CManager::GetInstance()->GetPlayData()->GetGameOver() == true)
		{
			//SE再生
	...
	//既定の値よりも大きくなったら
	if (m_nGameCounter > GAME03_NEXT_SCENE_COUNT)
	{
		//フェード取得処理
		CFade *pFade;
		pFade = CManager::GetInstance()->GetFade();
