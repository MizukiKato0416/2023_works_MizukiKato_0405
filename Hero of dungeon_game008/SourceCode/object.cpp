	...
	for (int nCntPriority = 0; nCntPriority < PRIORITY_MAX; nCntPriority++)
	{
		...
		while (pObject != nullptr)
		{
			...
			if (pObject->m_bDeath == false)
			{
				...
				{
					if (CManager::GetInstance()->GetPlayData()->GetGameClear() == true ||
						CManager::GetInstance()->GetPlayData()->GetGameOver() == true)
					{
						if (nCntPriority == PRIORITY_PAUSE || nCntPriority == PRIORITY_FADE || nCntPriority == PRIORITY_NONE)
						{
							pObject->Update();
						}
					}
