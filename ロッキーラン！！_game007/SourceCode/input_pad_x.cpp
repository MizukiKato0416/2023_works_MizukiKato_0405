...
bool CInputPadX::GetTriggerPress(int nTrigger, int nCntPad)
{
	...
	case TRIGGERTYPE_RIGHT:
		return m_astate[nCntPad].Gamepad.bLeftTrigger > XINPUT_GAMEPAD_TRIGGER_THRESHOLD ? true : false;
		break;
	case TRIGGERTYPE_LEFT:
		return m_astate[nCntPad].Gamepad.bRightTrigger > XINPUT_GAMEPAD_TRIGGER_THRESHOLD ? true : false;
		break;
	default:
		return false;
		break;
