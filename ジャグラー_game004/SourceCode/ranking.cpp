...
int g_nCol;			//透明にするよ～～～～
int g_nData;		//現在のスコア
int g_newRanking;	//ランキングに現在のスコアが載っていたらってやつだヨ！
bool g_bNewScan;	//ランキングに現在のスコアが載っていたらスキャンするヨ！
bool g_bCnt;		//順位がタイだった場合、上を光らせるよ～～～
...
void UpdateRanking(void)
{
	...
	for (nCntRanking = 0; nCntRanking < MAX_RANKING - 1; nCntRanking++)
	{
		for (int nCntScorePolygon = 0; nCntScorePolygon < MAX_SCORE_POLYGON; nCntScorePolygon++)
		{
			...
			if (g_bCnt == true)
			{
				if (g_aScore[nCntRanking] == g_nData && g_bNewScan == false)
				{
					g_bNewScan = true;
