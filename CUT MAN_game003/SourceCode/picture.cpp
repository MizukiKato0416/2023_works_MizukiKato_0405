...
void InitPicture(void)
{
	...
	for (nCntPicture = 0; nCntPicture < MAX_PICTURE; nCntPicture++)
	{
		...
		g_aPicture[nCntPicture].nType = PICTURETYPE_SETSUNEI;
...
void UpdatePicture(void)
{
	for (int nCntPicture = 0; nCntPicture < MAX_PICTURE; nCntPicture++)
	{
		if (g_aPicture[nCntPicture].bUse == true)
		{
			...
			{
				g_aPicture[nCntPicture].nType = PICTURETYPE_SOUSA;
			}
			else if (g_nCntPage == 2)
			{
				g_aPicture[nCntPicture].nType = PICTURETYPE_TAOSE;
			}
		}
	}
	...
	else if (GetButtonTrigger(XINPUT_GAMEPAD_B) == true && g_nCntPage != 2)
	{
		g_nCntPage++;
		PlaySound(SOUND_LABEL_SELECT);
	}
	else if((GetButtonTrigger(XINPUT_GAMEPAD_B) == true || GetButtonTrigger(XINPUT_GAMEPAD_START) == true) && g_nCntPage == 2 && GetFade() == FADE_NONE)
	{
