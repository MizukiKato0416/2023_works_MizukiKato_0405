...
void InitBlock(void)
{
	...
	for (nCntBlock = 0; nCntBlock < MAX_BLOCK; nCntBlock++)
	{
		...
		g_aBlock[nCntBlock].nCount = 0;
		g_aBlock[nCntBlock].nCount2 = 0;
...
void UpdateBlock(void)
{
	...
	for (nCntBlock = 0; nCntBlock < MAX_BLOCK; nCntBlock++)
	{
		...
		if (g_aBlock[nCntBlock].bUse == true)
		{
			...
			if (g_aBlock[nCntBlock].nType == 2)		//たいぷ２のブロック
			{
				g_aBlock[nCntBlock].nCount ++;
				if (g_aBlock[nCntBlock].nCount >= 700)
				{
					g_aBlock[nCntBlock].move.x *= -1.0f;
					g_aBlock[nCntBlock].nCount = 0;
				}
			}
			if (g_aBlock[nCntBlock].nType == 3)		//たいぷ3のブロック
			{
				g_aBlock[nCntBlock].nCount2++;
				if (g_aBlock[nCntBlock].nCount2 >= 500)
				{
		...
		if (g_aBlock[nCntBlock].pos.x <= SCREEN_WIDTH)
		{
